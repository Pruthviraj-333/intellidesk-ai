...
from app.utils.exceptions import (
    ConflictError, AuthenticationError, BusinessLogicError, ValidationError
)
...
class TestAuthServiceRegister:
    ...
    def test_register_raises_validation_error_if_no_role(self, app):
        with app.app_context():
            ...
            with patch("app.services.auth_service.UserRepository.email_exists", return_value=False):
                with patch("app.services.auth_service.RoleRepository.get_employee_role", return_value=None):
    ...
    def test_register_creates_user_and_sends_email(self, app):
        with app.app_context():
            ...
            with patch("app.services.auth_service.UserRepository.email_exists", return_value=False), \
                 patch("app.services.auth_service.RoleRepository.get_employee_role", return_value=mock_role), \
                 patch("app.services.auth_service.UserRepository.create", return_value=mock_user), \
                 patch("app.services.auth_service.UserTokenRepository.create_token", return_value="test_token"), \
                 patch("app.services.auth_service.EmailService.send_verification_email", return_value=True) as mock_email, \
...
class TestAuthServiceLogin:
    ...
    def test_login_raises_auth_error_for_wrong_password(self, app):
        with app.app_context():
            ...
            with patch("app.services.auth_service.UserRepository.get_by_email", return_value=mock_user):
...
class TestAuthServiceVerifyEmail:
    ...
    def test_verify_email_raises_on_invalid_token(self, app):
        with app.app_context():
            from app.services.auth_service import AuthService
            with patch("app.services.auth_service.UserTokenRepository.verify_token", return_value=None):
                with pytest.raises(ValidationError) as exc_info:
    ...
    def test_verify_email_raises_on_already_verified(self, app):
        with app.app_context():
            ...
            with patch("app.services.auth_service.UserTokenRepository.verify_token", return_value=mock_token), \
...
class TestAuthServicePasswordReset:
    ...
    def test_forgot_password_is_silent_for_nonexistent_email(self, app):
        with app.app_context():
            from app.services.auth_service import AuthService
            with patch("app.services.auth_service.UserRepository.get_by_email", return_value=None) as mock_get:

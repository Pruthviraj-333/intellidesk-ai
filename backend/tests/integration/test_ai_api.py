...
class TestAIChat:
    ...
    def test_employee_can_chat_with_ai(self, client, auth_headers_employee):
        with patch("app.services.ai_service.LLMService.chat_completion", return_value=MOCK_LLM_RESPONSE), \
             patch("app.services.ai_service.RAGService.semantic_search", return_value=MOCK_RAG_RESULTS), \
             patch("app.services.ai_service.RAGService.build_context_for_query", return_value="VPN context text."):
    ...
    def test_chat_continues_existing_session(self, client, auth_headers_employee):
        with patch("app.services.ai_service.LLMService.chat_completion", return_value=MOCK_LLM_RESPONSE), \
            ...
            resp1 = client.post("/api/v1/ai/chat", json={
    ...
    def test_chat_response_includes_rag_sources(self, client, auth_headers_employee):
        with patch("app.services.ai_service.LLMService.chat_completion", return_value=MOCK_LLM_RESPONSE), \
             patch("app.services.ai_service.RAGService.semantic_search", return_value=MOCK_RAG_RESULTS), \
             patch("app.services.ai_service.RAGService.build_context_for_query", return_value="Context"):
...
class TestAISessions:
    ...
    def test_get_session_history(self, client, auth_headers_employee):
        ...
        history_resp = client.get(f"/api/v1/ai/sessions/{session_uuid}", headers=auth_headers_employee)
    ...
    def test_delete_session(self, client, auth_headers_employee):
        ...
        del_resp = client.delete(f"/api/v1/ai/sessions/{session_uuid}", headers=auth_headers_employee)
...
class TestTicketClassification:
    ...
    def test_agent_can_classify_ticket(self, client, auth_headers_agent):
        ...
        with patch("app.services.llm_service.LLMService.classify_ticket", return_value=mock_classification):
            resp = client.post(
    ...
    def test_employee_cannot_classify_ticket(self, client, auth_headers_employee, auth_headers_agent):
        ticket_id = self._create_ticket(client, auth_headers_agent)
        resp = client.post(
            f"/api/v1/ai/tickets/{ticket_id}/classify",
            headers=auth_headers_employee,
    ...
    def test_submit_classification_feedback(self, client, auth_headers_agent):
        ...
        with patch("app.services.llm_service.LLMService.classify_ticket", return_value={
            "category": "Software", "priority": "medium",
            "department_name": "IT", "confidence": 0.75,
...
class TestTicketSuggestions:
    ...
    def test_suggest_response_for_ticket(self, client, auth_headers_agent):
        ...
        with patch("app.services.llm_service.LLMService.chat_completion", return_value=MOCK_LLM_RESPONSE), \
...
class TestResolutionGuide:
    ...
    def test_get_resolution_guide(self, client, auth_headers_employee):
        with patch("app.services.llm_service.LLMService.chat_completion", return_value=MOCK_LLM_RESPONSE), \
            ...
            resp = client.post("/api/v1/ai/resolution-guide", json={
